        ...
        {record && 
            ...
            >
                <header className="p-6">
                  <h1 className="font-creatoBold font-extraBold text-3xl">Record subscription</h1>
                  <p className="font-creato text-[lg] text-black-50">Document all depot subscription plans</p>
                </header>
                <section className="px-6 pb-4">
                  
                    <form onSubmit={form.onSubmit(() => setConfirmRecord(!confirmRecord))}>
                      <TextInput label="Depot" placeholder="Depot" {...form.getInputProps('depot')} styles={() => inputStyle} />
                      <TextInput mt="sm" label="Subscription Plan" placeholder="Subscription Plan" {...form.getInputProps('subscriptionPlan')} styles={() => inputStyle} component="select" rightSection={<FaAngleDown size={14} />}>
                          <option value="1">1</option>
                          <option value="2">2</option>
                      </TextInput>
                      <InputText label={"Subscription Amount"} value={""}/>
                      <InputText label={"Last Payment Date"} value={""} rightSection={<FaCalendarDay size={14} />} />
                      <TextInput label="New Payment Date" placeholder="New Payment Date" {...form.getInputProps('newPaymentDate')} styles={() => inputStyle} rightSection={<FaCalendarDay size={14} />} />
            ...
            >
                ...
                <div className="px-3 py-5">
                    ...
                    <p className="text-black-90 text-lg text-center md:mx-auto  px-3 md:px-0">
                      You are about to renew Revive traffic is subcription of £ 1,200 for October 2022
                    </p>
                </div>

                <div className="flex justify-between w-1/2 py-5 mx-auto mb-8">
                    <Button onClick={()=> setConfirmRecord(!confirmRecord) }>
                        Cancel
                    </Button>
                    <Button
                        variant="green"
